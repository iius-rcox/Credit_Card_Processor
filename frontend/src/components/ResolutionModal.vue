              ...
              <div class="space-y-2">
                ...
                <label class="flex items-center">
                  <input
                    v-model="form.action"
                    type="radio"
                    value="override_accept"
                    class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300"
                  />
                  <span class="ml-2 text-sm text-gray-900"
                    >Override/Accept</span
                  >
                </label>
                <label class="flex items-center">
                  <input
                  ...
                  />
                  <span class="ml-2 text-sm text-gray-900"
                    >Escalate to Manager</span
                  >
                </label>
              </div>
            </div>

            <!-- Amount Correction (if applicable) -->
            <div
              v-if="form.action === 'override_accept' && hasVariance"
              class="mb-4"
            >
              <label class="block text-sm font-medium text-gray-700 mb-2"
                >Correct Amount</label
              >
              <div class="grid grid-cols-2 gap-4">
                <div>
                  ...
                  >
                  <input
                    v-model.number="form.corrected_car_amount"
                    type="number"
                    step="0.01"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                    :placeholder="employee.car_amount"
                  />
                </div>
                <div>
                  <label class="block text-xs text-gray-500 mb-1"
                  ...
                  >
                  <input
                    v-model.number="form.corrected_receipt_amount"
                    type="number"
                    step="0.01"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
                    :placeholder="employee.receipt_amount"
                  />
                </div>
              </div>
            </div>
...
const form = ref({
  ...
  notes: '',
  priority: 'normal',
  corrected_car_amount: null,
  corrected_receipt_amount: null,
})
...
async function handleResolve() {
  ...
  try {
    ...
    // Add corrected amounts if provided
    if (form.value.corrected_car_amount !== null) {
      resolutionData.corrected_car_amount = form.value.corrected_car_amount
    }
    if (form.value.corrected_receipt_amount !== null) {
      resolutionData.corrected_receipt_amount =
        form.value.corrected_receipt_amount
    }
...
onMounted(() => {
  form.value.corrected_car_amount = props.employee.car_amount
  form.value.corrected_receipt_amount = props.employee.receipt_amount
})
</script>
